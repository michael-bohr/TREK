...
// confirm() checks the Budget addon before linking a cost; stub it so the
// dedupe/geocode tests below don't need a real db-backed admin lookup.
...
describe('BookingImportService.confirm — venue place dedupe', () => {
  ...
  it('geocodes an endpoint by its address when the name alone fails', async () => {
    ...
    // Transient geocode aid only — reservation_endpoints has no address column.
    expect(passed.endpoints[0]).not.toHaveProperty('address');
  });
});
