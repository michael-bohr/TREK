 ...
 * the SSRF check is rejected on BOTH the save path (addSource) and the
 * "Test connection" path (testConfig) — and, critically, rejected BEFORE
 * ImapProvider ever opens a socket to it (no constructor call, no
 * testConnection() call).
 */
