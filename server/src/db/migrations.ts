...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    // Migration 184: email ingestion (mail_ingest addon) — per-user IMAP sources
    // + an ingest log for dedupe/audit. subject/from_address are included from
    // the start (this is a brand-new table, unlike the older branch this was
    // ported from, which added them via a later ALTER). Additive; CREATE IF NOT
    // EXISTS so re-runs are safe. sort_order 17 — 16 is already taken by the
    // collections addon.
    () => {
      db.exec(`
