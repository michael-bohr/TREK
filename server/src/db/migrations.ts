...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    // Email ingestion (mail_ingest addon): per-user IMAP sources + an ingest log
    // for dedupe/audit. Additive; CREATE IF NOT EXISTS so re-runs are safe.
    () => {
      db.exec(`
      ...
      `);
      db.prepare(
        "INSERT OR IGNORE INTO addons (id, name, description, type, icon, enabled, sort_order) VALUES ('mail_ingest', 'Mail Ingest', 'Auto-import booking emails from your mailbox onto trips', 'integration', 'Mail', 0, 16)",
      ).run();
    },
  ];
